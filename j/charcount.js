...
function displayCharCounts(element)
{
	...
	if(remaining >= 0)
	{
		...
		limitLabel.innerHTML = remaining + ' Zeichen';
		if(remaining != 1)
			limitLabel.innerHTML += '';
		limitLabel.innerHTML += ' verbleiben';
	}
	else
	{
		if (enforceLength == true) {
			...
			limitLabel.style.color = 'green';
			limitLabel.innerHTML = '0 Zeichen verbleiben';
		}
		else {
			//Non-negative
			...
			limitLabel.innerHTML = '' + remaining + ' Zeichen zuviel';
			if (remaining != 1) 
